...
class PlayingField {
    ...
    /**
     * Private class used to memorize the cards and the points on which they are placed.
     */
    private static class PlacedCards {
        ...
        /**
         * Checks if a point is already occupied by a card.
         *
        ...
        /**
         * Retrieves the card at a given point. This method is only called after containsPoint(p) has returned true.
         *
         * @param p the point where the card is placed
         * @return the card that is placed at the given point
         */
        public synchronized PlayableCard getCardAt(Point p) {
            return cards.get(positions.indexOf(p));
        }

        /**
         * Returns an ArrayList used to iterate over points with placed cards.
         *
    ...
    /**
     * Creates the placedCards and visibleSymbols objects and sets the counter to 0 for every TokenType.
     */
    public PlayingField() {
    ...
    /**
     * Adds a card to the playing field after its validity is confirmed by the class Player.
     *
    ...
    /**
     * Updates the visible symbols on the playing field after placing a card.
     * Also calls coverCorners to remove any symbol that was covered during the placement.
     *
    ...
    private synchronized void coverCorners(Point p) throws NotPlacedException {
        ...
        if (placedCards.containsPoint(topRight)) { //If there is a card on the placed card's top right then that card's bottom left is what has been covered
            otherCardCoveredCorner = placedCards.getCardAt(topRight).getPlacedBottomLeft();
        ...
        if (placedCards.containsPoint(topLeft)) { //If there is a card on the placed card's top left then that card's bottom right is what has been covered
            otherCardCoveredCorner = placedCards.getCardAt(topLeft).getPlacedBottomRight();
            visibleSymbols.put(otherCardCoveredCorner, visibleSymbols.get(otherCardCoveredCorner) - 1);
        }
        if (placedCards.containsPoint(bottomLeft)) { //If there is a card on the placed card's bottom left then that card's top right is what has been covered
            otherCardCoveredCorner = placedCards.getCardAt(bottomLeft).getPlacedTopRight();
            visibleSymbols.put(otherCardCoveredCorner, visibleSymbols.get(otherCardCoveredCorner) - 1);
        }
        ...
        if (placedCards.containsPoint(bottomRight)) { //If there is a card on the placed card's bottom right then that card's top left is what has been covered
            otherCardCoveredCorner = placedCards.getCardAt(bottomRight).getPlacedTopLeft();
            visibleSymbols.put(otherCardCoveredCorner, visibleSymbols.get(otherCardCoveredCorner) - 1);
        }
    }
     ...
     *
     * @param goldCard the Gold card to check
     ...
     * @throws NotPlacedException if a fatal initialization error has occurred
     */
    public synchronized boolean isPositionAvailable(Point point) throws NotPlacedException {
        ...
        if (placedCards.containsPoint(topRight)) {
            countAdjacenntCards++;
            if (placedCards.getCardAt(topRight).getPlacedBottomLeft() == TokenType.blocked) {
                return false;
            }
        }
        ...
        if (placedCards.containsPoint(topLeft)) {
            countAdjacenntCards++;
            if (placedCards.getCardAt(topLeft).getPlacedBottomRight() == TokenType.blocked) {
                return false;
            }
        }
        ...
        if (placedCards.containsPoint(bottomRight)) {
            countAdjacenntCards++;
            if (placedCards.getCardAt(bottomRight).getPlacedTopLeft() == TokenType.blocked) {
                return false;
            }
        }
        ...
        if (placedCards.containsPoint(bottomLeft)) {
            countAdjacenntCards++;
            if (placedCards.getCardAt(bottomLeft).getPlacedTopRight() == TokenType.blocked) {
                return false;
            }
        }
    ...
    /**
     * Excludes "empty" and "blocked" from the result.
     *
    ...
    /**
     * Checks how the card awards points, then calculates how many points are scored.
     * This method is only called after placing a card.
     *
     * @param card the gold card that has just been placed
     * @return the number of points the card scored for the player
    ...
    /**
     * Calculates how many points are awarded by an objective card.
     *
    ...
    /**
     * Counts how many diagonals of the needed color are there.
     *
     * @param requirements used to determine if the program is searching for a blue or red diagonal
     * @param xDirection   the direction in which the x coordinate of the diagonal moves, from bottom to top so the y increases by 1 always
     * @return the number of red or blue diagonals
     */
    private synchronized int countDiagonals(ObjectiveSequence requirements, int xDirection) {
        ...
        for (Point p : placedCards.pointsList()) {
            if (placedCards.getCardAt(p).getCardColour() == colour) {
    ...
    /**
     * Counts how many L-shapes are present that match the given criteria.
     *
     * @param columnColour the colour of the "|" part of the L-Shape
     * @param sideColour   the colour of the "_" part of the L-shape
     ...
     * @return the number of L-shapes that satisfy the parameters
     */
    private synchronized int countLShapes(CardType columnColour, CardType sideColour, int xOffset, int yOffset) {
        ...
        for (Point p : placedCards.pointsList()) {
            if (placedCards.getCardAt(p).getCardColour() == columnColour) {
        ...
        for (Point p : placedCards.pointsList()) {
            ...
            */
            if (placedCards.getCardAt(p).getCardColour() == sideColour) {
    ...
    /**
     * Updates the list of extremities and points that should be removed from the possible sequence points.
     *
     * @param toRemove                the list of points to remove from the possible sequence points
     * @param possibleSequencePoints  the list of possible sequence points
     * @param topExtremities          the list of top extremities
     * @param bottomExtremities       the list of bottom extremities
     * @param topAdj                  the point above the current point
     * @param bottomAdj               the point below the current point
    ...
    /**
     * Finds the sequences starting from the given extremities.
     *
     * @param startingExtremities the list of starting extremities
     * @param endingExtremities   the list of ending extremities
     * @param possibleSidePoints  the list of possible side points
     * @param possibleColumnPoints the list of possible column points
     * @param xOffset             the x offset
     * @param direction           the direction (top-to-bottom or bottom-to-top)
