...
class PlayingField {
    /**
     * Private class used to memorize the cards and the points on which they are laced.
     */
    ...
    /**
     * Checks if a card can be placed on the given point. <br>A point (z,w) is valid only if currently there is no card on it and there is at least a point (x,y)
     * such that (x+1,y+1)=(z,w) or (x-1,y+1)=(z,w) or (x+1,y-1)=(z,w) or (x-1,y-1)=(z,w) that has a card placed on it.
     * And there aren't cards that: have a blocked bottomLeftCorner in (z+1,w+1); have a blocked bottomRightCorner in (z-1,w+1);
     * have a blocked topRightCorner in (z-1,w-1); have a blocked topLeftCorner in (z+1,w-1)
     *
    ...
    /**
     * "empty","blocked" are excluded from the result.
     *
